...
function log(){
    if ! tty -s; then logger -s -t "${SCRIPT_NAME}" -p "$@"; else echo "$@" >&2; fi;
}

function lookup_path_in_instance_metadata {
    local readonly path="$1"
    curl --fail --silent --show-error --location "${INSTANCE_METADATA_URL:-http://169.254.169.254/latest/meta-data}/$path/"
}
